...
class FirstSheet implements ToModel, WithStartRow,WithCalculatedFormulas
{
    ...
    public function model(array $row)
    {
        return new Penduduk([
            ...
            "waktu_kematian" => ($row[25] == null ? $row[25] : ($row[25] < 1 ? intdiv($row[25] * 24 * 60, 60) . '.' . ($row[25] * 24 * 60 % 60) : $row[25])),
            ...
            "nik_ibu" => $row[31],
            "nik_ayah" => $row[32],
            "tanggal_cerai" => ($row[33] == null ? $row[33] : Carbon::parse(((int)($row[33]) - 25569) * 86400)->format("Y-m-d")),
